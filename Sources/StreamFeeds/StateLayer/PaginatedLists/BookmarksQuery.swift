...
/// A filter field for bookmarks that defines how filter fields are represented as strings.
///
...
extension BookmarksFilterField {
    ...
    public static let activityId = Self("activity_id", localValue: \.activity.id)
    
    /// Filter by the unique identifier of the bookmark folder.
    ///
    /// **Supported operators:** `.equal`, `.in`, `.exists`
    public static let folderId = Self("folder_id", localValue: \.folder?.id)
...
/// A sortable field for bookmarks that can be used for both local and remote sorting.
///
